...
import { PokemonSpecieService } from '@services/pokemon/pokemonSpecieService';
import { PokemonStatService } from '@services/pokemon/pokemonStatService';
import { PokemonTypeService } from '@services/pokemon/pokemonTypeService';
import { PokemonAbilityService } from '@services/pokemon/pokemonAbilityService';
import { PokemonEvolutionService } from '@services/pokemon/pokemonEvolutionService';
import { PokemonMoveService } from '@services/pokemon/pokemonMoveService';
import PokemonApi from '@api/pokemon.api';
import PokemonRepository from '@repositories/pokemon/PokemonRepository';
import PokemonMapper from '@mapper/pokemon/pokemonMapper';
...
export class PokemonService extends BaseService<Pokemon, IPokemon> {
    ...
    async generatePokemons({
        ...
        const result = await Promise.all(list.results.map(async (item: IPokemonBase) => {
            const order = StringUtils.generateOrder(item.url, 'https://pokeapi.co/api/v2/pokemon/');
            const specie = await this.initializeDatabaseSpecie(order, item.name);
            if(!specie) {
                return;
            }
    ...
    private async initializeDatabaseSpecie(order: number = 0, name: string = ''): Promise<PokemonSpecies | undefined> {
        const speciesService = new PokemonSpecieService();
        return await speciesService.generateDefaultPokemonSpecie(order, name);
    }
